...
crypto_test_epoch_setup(void **state)
{
    ...
    struct epoch_key epoch1 = { .epoch = 1, .epoch_key = {0x23} };

    epoch_init_key_ctx(&data->co, &data->kt, KEY_DIRECTION_NORMAL, &epoch1, *num_future_keys);
...
crypto_test_epoch_key_receive_lookup(void **state)
{
    ...
    assert_null(lookup_decrypt_epoch_key(co, 2000));
    assert_null(lookup_decrypt_epoch_key(co, -1));
    assert_null(lookup_decrypt_epoch_key(co, 0xefffff));

    /* Lookup the edges of the current window */
    assert_null(lookup_decrypt_epoch_key(co, 0));
    assert_int_equal(lookup_decrypt_epoch_key(co, 1)->epoch, 1);
    assert_int_equal(lookup_decrypt_epoch_key(co, 2)->epoch, 2);
    assert_int_equal(lookup_decrypt_epoch_key(co, 13)->epoch, 13);
    assert_int_equal(lookup_decrypt_epoch_key(co, 14)->epoch, 14);
    assert_null(lookup_decrypt_epoch_key(co, 15));
    ...
    assert_null(lookup_decrypt_epoch_key(co, 0));
    assert_int_equal(lookup_decrypt_epoch_key(co, 1)->epoch, 1);
    assert_null(lookup_decrypt_epoch_key(co, 2));
    assert_null(lookup_decrypt_epoch_key(co, 3));
    assert_null(lookup_decrypt_epoch_key(co, 4));
    assert_null(lookup_decrypt_epoch_key(co, 5));
    assert_null(lookup_decrypt_epoch_key(co, 6));
    assert_int_equal(lookup_decrypt_epoch_key(co, 7)->epoch, 7);
    assert_int_equal(lookup_decrypt_epoch_key(co, 8)->epoch, 8);
    assert_int_equal(lookup_decrypt_epoch_key(co, 20)->epoch, 20);
    assert_null(lookup_decrypt_epoch_key(co, 21));
    assert_null(lookup_decrypt_epoch_key(co, 22));
    ...
    assert_int_equal(lookup_decrypt_epoch_key(co, 8)->epoch, 8);
    assert_int_equal(lookup_decrypt_epoch_key(co, 20)->epoch, 20);
    assert_int_equal(lookup_decrypt_epoch_key(co, 21)->epoch, 21);
    assert_null(lookup_decrypt_epoch_key(co, 22));
    assert_null(lookup_decrypt_epoch_key(co, 23));
}
...
crypto_test_epoch_key_overflow(void **state)
{
    ...
    assert_int_equal(lookup_decrypt_epoch_key(co, UINT16_MAX - 34)->epoch, UINT16_MAX - 34);
    assert_int_equal(lookup_decrypt_epoch_key(co, UINT16_MAX - 33)->epoch, UINT16_MAX - 33);

    /* This key is no longer eligible for decrypting as the 32 future keys
     * would be larger than uint16_t maximum */
    assert_null(lookup_decrypt_epoch_key(co, UINT16_MAX - 32));
    assert_null(lookup_decrypt_epoch_key(co, UINT16_MAX));
...
void
crypto_test_epoch_key_iterate_send_key(void **state)
{
...
main(void)
{
    ...
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_prestate_setup_teardown(crypto_test_epoch_key_overflow,
                                                 ...
                                                 crypto_test_epoch_teardown,
                                                 &prestate_num32),
        cmocka_unit_test_prestate_setup_teardown(crypto_test_epoch_key_iterate_send_key,
                                                 crypto_test_epoch_setup,
                                                 crypto_test_epoch_teardown,
                                                 &prestate_num13)
    };
