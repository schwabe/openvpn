...
static void
free_dco_ctx(dco_context_t *dco)
{
    if (!dco)
    {
        return;
                                            ...
                                            enum ovpn_nl_commands cmd)
{
    ...
    nla_put_failure:
    nlmsg_free(nl_msg);
    msg(M_ERR, "cannot put into netlink message");
    return NULL;
}
...
static int ovpn_nl_recvmsgs(dco_context_t *dco, const char *prefix)
{
    ...
    case -NLE_OBJ_NOTFOUND:
        msg(M_ERR, "%s: netlink reports object not found, ovpn-dco unloaded?", prefix);
        break;
    default:
        if (ret)
        {
...
static void ovpn_dco_register(dco_context_t *dco)
{
    ...
    /* Register for Ovpn dco specific messages and to RTNLGRP_LINK to
     * receive RTM_DELLINK */
    int ret = nl_socket_add_memberships(dco->nl_sock, dco->ovpn_dco_mcast_id,
                                        RTNLGRP_LINK, RTNLGRP_NOTIFY, RTNLGRP_NONE);
    if (ret)
    {
...
open_tun_dco(struct tuntap *tt, const char* dev)
{
    ...
    dco->ifindex = net_iface_new(dev, "ovpn-dco");

    if(!if_indextoname(dco->ifindex, if_name))
    {
...
close_tun_dco(struct tuntap *tt)
{
    ...
    net_iface_del_index(dco->ifindex);
    ovpn_dco_uninit_netlink(dco);
    free(dco->dco_packet_in.data);
}
             ...
             struct key_state *ks, const char* ciphername)
{
    ...
    NLA_PUT(nl_msg, OVPN_KEY_DIR_ATTR_NONCE_TAIL, nonce_len,
            key->encrypt.implicit_iv + nonce_offset);
    ...
    NLA_PUT(nl_msg, OVPN_KEY_DIR_ATTR_NONCE_TAIL, nonce_len,
            key->decrypt.implicit_iv + nonce_offset);
