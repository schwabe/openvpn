...
bool
management_send_acc_message(struct tls_multi *multi, struct tls_session *session,
                            const char *negotiated_protocols, struct buffer_list *input)
{
    ...
    if (!tls_item_in_cipher_list(protocol, negotiated_protocols))
    {
        msg(M_CLIENT, "ERROR: protocol not in the list of negotiated protocols: %s", negotiated_protocols);
        goto error;
    }
