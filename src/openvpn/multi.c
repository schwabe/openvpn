...
multi_client_set_protocol_options(struct context *c)
{
    ...
    if (strlen(peer_ciphers) > 0)
    {
        ...
        /* we are in a situation where we know the client ciphers, there is no
         * reason to fall back to a cipher that will not be accepted by
         * the other side*/

        auth_set_client_reason(tls_multi, ncp_auth_failed);
    }
    else
    {
        msg(M_INFO, "No NCP data received from peer. "
