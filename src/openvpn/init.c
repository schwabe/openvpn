...
#elif defined(ENABLE_LINUXDCO)
    /* Store the link socket in the dco context for now as hacky workaround */
    c->c1.tuntap->dco_ctx.link_socket = c->c2.link_socket;
#endif
    /* open the tun device */
...
p2p_dco_add_new_peer(struct context *c)
{
    ...
    const char* gw = NULL;
    /* In client mode we assume the remote-gateway is the IP of the peer */
    ...
    /* These inet_pton conversion are fatal since options.c should already
    ...
    if (!c->c2.link_socket->info.dco_installed)
    {
        ...
        dco_new_peer(dco, multi->peer_id, c->c2.link_socket->sd, remoteaddr,
    ...
    if (c->options.ping_send_timeout)
    {
        ovpn_set_peer(dco, multi->peer_id, c->options.ping_send_timeout,
...
static void remove_dco_peer(struct context *c)
{
    ...
    {
        dco_del_peer(&c->c1.tuntap->dco_ctx, c->c2.tls_multi->peer_id);
    }
}
#else
static void remove_dco_peer(struct context *c)
