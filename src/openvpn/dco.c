...
bool
dco_do_key_dance(struct dco_context *dco, struct tls_multi *multi, const char *ciphername)
{
    ...
    const char * print_key_id(struct tls_multi *multi, struct gc_arena *gc);

    int pid = primary->key_id;
    int sid = secondary ? secondary->key_id : -1;

    msg(D_DCO_DEBUG, "Doing the key dance with peer %d and keys %d and %d!",
        multi->peer_id, pid, sid);

    /* General strategy, get primary installed correctly first. If that is
     * okay then check if we need to exchange secondary */
    if (primary->dco_status != DCO_INSTALLED_PRIMARY)
    {
        if (primary->dco_status != DCO_INSTALLED_SECONDARY)
        {
            dco_new_key(dco, multi->peer_id, OVPN_KEY_SLOT_SECONDARY,  primary, ciphername);
        }
    ...
    {
        dco_new_key(dco, multi->peer_id, OVPN_KEY_SLOT_SECONDARY, secondary, ciphername);
        secondary->dco_status = DCO_INSTALLED_SECONDARY;
        multi->dco_keys_installed = 2;
    }
    else if (!secondary && multi->dco_keys_installed == 2)
    {
        dco_del_key(dco, multi->peer_id, OVPN_KEY_SLOT_SECONDARY);
        multi->dco_keys_installed = 1;
    }
