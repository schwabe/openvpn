...
static void show_compression_warning(struct compress_options *info)
{
    if (comp_non_stub_enabled(info))
    {
        ...
        {
            msg(M_WARN, "WARNING:  Compression enabled, Compression has been used in the past to "
                "break encryption. Enabling decompression of received packet only. "
...
/*      VERIFY_PERMISSION (OPT_P_MTU); */
    ...
    else if (streq(p[0], "allow-compression") && p[1] && !p[2])
    {
        ...
        {
            msg(M_WARN, "WARNING: Compression enabled, Compression has been used in the past to break encryption. "
